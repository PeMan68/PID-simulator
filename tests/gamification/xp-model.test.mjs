// GAM-003A — DEL 17: Automatiska tester för XP-beräkningsverktyget.
...
function run(events, rules) { return computeXP(events, rules || XP_RULES_V1); }
const CTX = "t#0";
...
// 6. Distinkt konfiguration ger XP först efter genomfört försök
{
  const events = [step(0), paramChange("kp", 5), runN(20)]; // ingen finalisering ännu (ingen reset/kontextbyte/param-ändring efter)
  const r = run(events);
  check("6a. Ingen distinctConfig-XP innan försöket finaliserats", r.byCategory.distinctConfig === 0, "fick " + r.byCategory.distinctConfig);
  const events2 = [...events, { type: "system_reset", meta: { contextKey: CTX } }];
  const r2 = run(events2);
  check("6b. distinctConfig-XP ges när försöket finaliseras (Reset)", r2.byCategory.distinctConfig === XP_RULES_V1.distinctConfig);
}
